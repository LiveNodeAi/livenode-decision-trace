...
- Every non-inference evidence value, link `label`, and link `sourceExcerpt` must occur in the memo after NFKC normalization and whitespace collapse/trim only; case, punctuation, and ellipses remain exact. Link `relationship` remains AI synthesis.
